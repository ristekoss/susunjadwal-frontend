  ...
  courseList.forEach((course) => {
    ...
    classes.forEach((classItem) => {
      ...
      if (classKey in scheduleMap && !(classKey in added)) {
        ...
        savedSchedule.schedule_items = savedSchedule.schedule_items.filter(
          (sched) =>
            sched.name !== classItem.name && sched.course_name !== course.name,
        );
      }
    });
  });
...
export const generateScheduledCourseListFromSchedule = async (
  majorId,
  schedule,
) => {
  const { data: courses } = await getCourses(majorId);
  let [formattedSchedule, remainingSchedule] = formatScheduleFromCourse(
    courses,
    schedule,
  );
  remainingSchedule.schedule_items.forEach((sched) => {
    formattedSchedule.push({
      parentName: sched.course_name,
      term: sched.term,
      credit: sched.sks,
      name: sched.name,
